"""MDP curriculum, reward, and observation functions for G1 loco-manipulation tasks."""
...
def disturbance_range_curriculum(
    ...
    """Unified curriculum to increase both push force and external force/torque intensity.

    Increases disturbance based on termination rate and reward performance.
    ...
    current_push_magnitude = 0.1
    current_force_magnitude = 1.0

    # Increase disturbance if both conditions are met:
...
def foot_clearance_reward(
    ...
    foot_horizontal_vel = foot_velocities_w[:, :, :2]  # [num_envs, num_feet, 2]
...
def torso_orientation_l2(
    ...
    # Third column of rotation matrix (z-axis in body frame)
    r13 = 2 * (quat_x * quat_z + quat_w * quat_y)
    r23 = 2 * (quat_y * quat_z - quat_w * quat_x)
    r33 = quat_w**2 - quat_x**2 - quat_y**2 + quat_z**2
...
def torso_backward_tilt_penalty(
    ...
    backward_tilt = torch.clamp(-pitch_approx, min=0.0)
