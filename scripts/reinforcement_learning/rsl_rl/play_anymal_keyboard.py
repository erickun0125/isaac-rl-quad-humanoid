...
"""Script to play a checkpoint of an RL agent with keyboard control for Anymal-C locomotion."""
...
class AnymalKeyboardDemo:
    """This class provides an interactive demo for Anymal-C locomotion with keyboard control.
    ...
    * ESC: exit current third-person view
    ...
    def __init__(self, task_name: str, args_cli):
        ...
        if args_cli.video:
            video_kwargs = {
                "video_folder": "videos/anymal_keyboard_play",
    ...
    def setup_keyboard(self):
        ...
        # Define maximum values for each command type
        self._max_values = {
            "linear_x": 3.5,
            "linear_y": 1.5, 
            "angular_z": 2.5
        ...
        self._acceleration_rates = {
            "linear_x": 1.0,  # Reach max in 0.5 seconds
            "linear_y": 1.0,   # Reach max in 0.5 seconds  
            "angular_z": 2.0   # Reach max in 0.5 seconds
        }
        ...
        self._key_mappings = {
            "UP": (0, 1.0),      # linear_x positive
            "DOWN": (0, -1.0),   # linear_x negative  
            "LEFT": (1, 1.0),    # linear_y positive
            "RIGHT": (1, -1.0),  # linear_y negative
            "Z": (2, 2.0),       # angular_z positive (turn left)
    ...
    def run(self):
        """Main execution loop."""
        print("\n" + "="*50)
        print("Anymal-C Keyboard Control Demo")
        print("="*50)
        print("Keyboard Controls:")
        print("  UP Arrow    : Move forward (max speed: 5.0 m/s)")
        print("  DOWN Arrow  : Move backward (max speed: -5.0 m/s)")
        print("  LEFT Arrow  : Move left (max speed: 2.5 m/s)")
        print("  RIGHT Arrow : Move right (max speed: -2.5 m/s)")
        print("  Z           : Turn left (max angular: 2.5 rad/s)")
        print("  X           : Turn right (max angular: -2.5 rad/s)")
        ...
        while simulation_app.is_running():
            ...
            try:
                # Set commands using the command manager (similar to H1 demo approach)
                if hasattr(self.env.unwrapped, 'command_manager'):
                    # For environments with non-zero command indices, set specific robot commands
                    if self._selected_id is not None:
            ...
            with torch.inference_mode():
                ...
                # The velocity commands are typically at indices 9:12 in the observation
                try:
                    obs[:, 9:12] = self.commands
                except Exception as e:
...
def main():
    ...
    demo = AnymalKeyboardDemo(task_name, args_cli)
    demo.run()
