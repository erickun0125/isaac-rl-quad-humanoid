...
def g1_locomanip_symmetry_with_ee(env, obs, actions, obs_type="policy"):  # noqa: ARG001
    """
    G1 humanoid robot symmetry function for loco-manipulation environment with end-effector pose commands.
...
def mirror_observations_with_ee(obs, obs_type):
    """Mirror observations for left-right symmetry transformation including end-effector pose commands"""
    mirrored = obs.clone()
    ...
    if obs_type == "policy":
        ...
        # Critic observations WITH EE POSE COMMANDS
...
if __name__ == "__main__":
    # Policy observation test WITH EE POSE COMMANDS (estimated dimensions for G1 loco-manipulation)
    batch_size = 4
    policy_obs_dim = 197  # 3 + 3 + 3 + 7 + 7 + 87 + 58 + 29
    ...
    mirrored_obs, mirrored_actions = g1_locomanip_symmetry_with_ee(
    ...
    # Critic observation test WITH EE POSE COMMANDS (estimated dimensions)
    critic_obs_dim = 203  # 3 + 1 + 2 + 3 + 3 + 3 + 7 + 7 + 87 + 58 + 29
    critic_obs = torch.randn(batch_size, critic_obs_dim)
    mirrored_critic_obs, _ = g1_locomanip_symmetry_with_ee(
    ...
    print("\n=== G1 Humanoid Robot Joint Symmetry Mapping WITH EE POSE COMMANDS ===")
    ...
    print("  left_ee_pose_command ↔ right_ee_pose_command")
    ...
    # 
    # Note: Observation dimensions WITH EE POSE COMMANDS:
    # - Policy obs: base_ang_vel(3) + projected_gravity(3) + velocity_commands(3) + 
    #               left_ee_pose_command(7) + right_ee_pose_command(7) +
    #               joint_pos_history(29*3) + joint_vel_history(29*2) + action_history(29*1)
    #               = 3 + 3 + 3 + 7 + 7 + 87 + 58 + 29 = 197 dims
    # - Critic obs: base_lin_vel(3) + base_pos_z(1) + foot_contact(2) + base_ang_vel(3) + 
    #               projected_gravity(3) + velocity_commands(3) + left_ee_pose_command(7) + 
    #               right_ee_pose_command(7) + joint_pos_history(29*3) + joint_vel_history(29*2) + 
    #               action_history(29*1)
    #               = 3 + 1 + 2 + 3 + 3 + 3 + 7 + 7 + 87 + 58 + 29 = 203 dims
    """)
